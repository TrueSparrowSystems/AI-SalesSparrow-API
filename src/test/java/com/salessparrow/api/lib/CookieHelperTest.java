...
public class CookieHelperTest {
  ...
  void testGetCookieValue_ValidUser() {
    ...
    when(localCipher.decrypt(anyString(), anyString())).thenReturn(decryptedSaltValue);
    ...
    // Act
    String cookieValue = cookieHelper.getCookieValue(user, decryptedSaltValue, timestamp);
    System.out.println("cookieValue: " + cookieValue);

    // Assert
    String expectedCookieValue = CookieConstants.LATEST_VERSION + ':' + userId + ':' + timestamp + ':' + cookieToken;
  ...
  void testGetCookieToken_ValidUser() throws Exception {
    ...
    when(localCipher.decrypt(anyString(), anyString())).thenReturn(decryptedSalt);
    when(coreConstants.apiCookieSecret()).thenReturn("apiSecret");
    when(localCipher.encrypt(anyString(), anyString())).thenReturn(encryptedCookieToken);

    // Act
    String cookieToken = cookieHelper.getCookieToken(user, decryptedSalt, timestamp);
  ...
  void testSetCookieInHeaders_ValidCookieValue() {
    ...
    // Act
    HttpHeaders resultHeaders = cookieHelper.setCookieInHeaders(cookieName, cookieValue, cookieExpiryInMs, headers);

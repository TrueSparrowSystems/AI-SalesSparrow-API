...
/**
 * SalesforceCompositeResponseHelper is a class for handling the response from the
 * Salesforce API.
 **/
@Component
	...
	public SalesforceErrorObject getErrorObjectFromCompositeResponse(JsonNode responseBody) {
		...
		for (int i = 0; i < CompositeResponse.size(); i++) {
			...
			if (httpStatusCode != 200 && httpStatusCode != 201 && httpStatusCode != 204) {
				...
				CompositeResponseDto compositeResponseBody = mapper.convertValue(CompositeResponseItem,
						CompositeResponseDto.class);

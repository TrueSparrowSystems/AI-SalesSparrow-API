...
public class CookieHelper {
  ...
  public String getCookieValue(User user, String decryptedSalt, Long timestamp){
    ...
    return CookieConstants.LATEST_VERSION + ':' + user.getId() + ':' + timestamp + ':' + cookieToken;
  }
  ...
  public String getCookieToken(User user, String decryptedSalt, Long timestamp){
    ...
    String stringToSign = user.getId() + ':' + timestamp + ':' + strSecret + ':' + decryptedCookieToken.substring(0, 16);
    String salt = user.getId() + ':' + decryptedCookieToken.substring(decryptedCookieToken.length()-16) + ':' + strSecret + ':' + timestamp;
    String encryptedCookieToken = localCipher.encrypt(salt, stringToSign);

    return encryptedCookieToken;
  }

  public HttpHeaders setCookieInHeaders(String cookieName, String cookieValue, int cookieExpiryInMs, HttpHeaders headers) {
